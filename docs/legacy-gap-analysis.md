...
- 旧权限节点中的导入/导出已专项核对：`equipment/archive/import` 和 `equipment/archive/exportTag` 已分别落到设备档案 CSV 导入、设备标签 CSV 导出。
- 新增后台 CSV 导出接口，覆盖设备档案、设备台账、维修工单、计划任务；前端档案页已接入真实导入、导出、导出标签按钮。

已收敛但不纳入第一版业务等价范围：

- FastAdmin 的回收站、恢复、销毁、选择器等框架型后台能力不影响当前设备工单核心流程，第一版不做完全复刻。
