...
- FastAdmin 的全部按钮级权限、回收站、导入导出、选择器等非核心后台能力尚未完全等价。
- 微信真实 `code -> openid` 需要有效小程序 AppID/Secret；测试环境仍支持直接传 `openid` 或 `code=openid:<openid>` 验证流程。
- 附件文件实体仍沿用旧路径字段保存；如要迁移文件到新目录或对象存储，需要单独执行文件校验和路径替换计划。
