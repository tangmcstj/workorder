...
- 旧后台三个管理员账号已对齐到新版登录：`admin`、`test`、`rsound` 均使用 FastAdmin `fa_admin` 密码规则校验。`admin` 和 `rsound` 属超级管理员组，显示 14 个新版菜单；`test` 属二级管理组，显示 13 个新版菜单且不显示“用户角色”。新版路由已增加菜单权限拦截，隐藏菜单不能通过直接输入路径访问。
